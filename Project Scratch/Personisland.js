import { CharacterControls, CONTROLLER_BODY_RADIUS } from './personisland/characterControl.js';
import { KeyDisplay } from './personisland/keyDisplay.js';
import { RigidBody, World } from '@dimforge/rapier3d';
import * as THREE from './mrdoob-three.js/build/three.module.js';
import { AmbientLight, BoxBufferGeometry, MeshPhongMaterial } from './mrdoob-three.js/build/three.module.js';
...
Promise.all([import('@dimforge/rapier3d'),import('./mrdoob-three.js/build/three.module.js'),
  import('./mrdoob-three.js/build/OrbitControls.js'),import('./mrdoob-three.js/build/GLTFLoader.js')
]).then(([RAPIER, THREE]) => {
  function body(scene, world, bodyType, colliderType, dimension, translation, rotation, color) {
    ...
    if (colliderType === 'cube') {
      ...
      // cone center of mass is at the bottom
  ...
  function generateTerrain(nsubdivs, scale) {
    ...
    // store height data in a column-row map
  ...
  new THREE.GLTFLoader().load('models/Soldier.glb', function (gltf) {
  ...
  let gameLoop = () => {
    ...
    world.step();
    // update 3d world with the physical world
    bodys.forEach(body => {
